...
const ProductCardItem = ({ product }) => {
  ...
  const handleAddToCart = async () => {
    if (!isDragged) {
      setIsDragged(true);
      try {
        const defaultSize = product.sizes?.[0]?._id || null;
        const defaultColor = product.colors?.[0]?._id || null;
        await addToCart(product._id, defaultSize, defaultColor, 1);
      } catch (err) {
        console.error(err);
      } finally {
        setTimeout(() => {
  ...
  const displaySubtitle = product.description
    ? product.description.length > 30
      ? product.description.slice(0, 30) + "..."
      : product.description
    : "Premium garment drop.";
  const displayPrice = `₹${(product.price?.saleAmount || product.price?.amount || 0).toLocaleString("en-IN")}`;

  return (
    <div
      className={`bg-surface dark:bg-[#1C1C1E] text-foreground p-3 rounded-[2.5rem] w-[240px] shadow-md lg:shadow-2xl relative border border-white/10 dark:border-white/10 transition-colors flex-shrink-0 snap-center`}
    >
      ...
      <div className="px-2 pb-2 text-center">
        ...
        <div
          ...
          style={{
            backgroundColor: isDragged ? "var(--color-acc)" : "transparent",
