...
import { setCart, toggleCartDrawer, setCartDrawerOpen, setLoading, setError } from "../State/cart.slice";
...
export const useCart = () => {
    ...
    const addToCart = async (productId, sizeId = null, colorId = null, quantity = 1) => {
        if (!user) {
        ...
        dispatch(setLoading(true));
        try {
            const data = await api.addItemToCart(productId, sizeId, colorId, quantity);
            dispatch(setCart(data.cart));
    ...
    return { getCart, addToCart, updateQuantity, removeFromCart, clearCart, toggleDrawer, setDrawerOpen };
};
