...
const CreateProduct = () => {
  ...
  const allVariantKeys = useMemo(() => {
    ...
    if ((form.sizes.length > 0 || metadata.sizes?.length > 0) && !keys.includes('size')) keys.push('size');
    if ((form.colors.length > 0 || metadata.colors?.length > 0) && !keys.includes('color')) keys.push('color');
    ...
    return keys;
  }, [customAttrKeys, form.sizes, form.colors, metadata.sizes, metadata.colors, globalAttributes]);
                      ...
                      <div key={index} className="bg-surface/50 border border-border-theme/60 p-6 rounded-2xl space-y-4 relative">
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
                          {allVariantKeys.map(k => {
                            let options = null;
                            if (k === 'size') {
                              options = metadata.sizes?.map(s => s?.name).filter(Boolean);
                              ...
                              options = metadata.colors?.map(c => c?.name).filter(Boolean);
                              if (!options || options.length === 0) {
