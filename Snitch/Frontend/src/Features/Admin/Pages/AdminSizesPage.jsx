...
const AdminSizesPage = () => {
    const { sizes, categories, loading } = useSelector(state => state.admin);
    const { fetchAll, handleCreateSize, handleUpdateSize, handleDeleteSize } = useAdmin();
    ...
    const handleAction = async (e) => {
        ...
        try {
            if (editItem) {
                await handleUpdateSize(editItem._id, formData);
            } else {
                await handleCreateSize(formData);
            }
            resetForm();
    ...
    const confirmDelete = () => {
        if (deleteModal.id) {
            handleDeleteSize(deleteModal.id);
        }
    ...
    return (
        <div className="space-y-8 animate-in fade-in duration-500">
            <Modal 
                ...
                title="Delete Size?"
                description="Are you sure you want to remove this size? This action is permanent and cannot be undone."
            ...
            <header className="flex flex-col md:flex-row md:items-end justify-between gap-6">
                <div>
                    ...
                    <h1 className="text-5xl font-black tracking-tighter text-foreground mt-1">Sizes</h1>
                    <p className="text-foreground/40 mt-2">Manage sizing standards across different categories.</p>
                </div>
                <PrimaryBtn icon="ri-add-line" onClick={() => setShowForm(true)}>Add Size</PrimaryBtn>
            </header>
            ...
            <div className="relative">
                ...
                <input
                    ...
                    placeholder="Search sizes by name or category…"
                ...
                <div className="fixed inset-0 z-[100] flex items-center justify-center bg-background/80 backdrop-blur-xl p-4">
                    <div className="bg-surface border border-border-theme rounded-3xl p-8 w-full max-w-md shadow-2xl animate-in zoom-in-95 duration-300">
                        <h2 className="text-2xl font-black mb-6">
                            {editItem ? 'Edit' : 'New'} Size
                        </h2>
                        <form onSubmit={handleAction} className="space-y-4">
                            <div>
                                <label className="text-[10px] font-black uppercase text-foreground/40 ml-1">Size Name</label>
                                <input required value={formData.name || ''} onChange={e => setFormData({...formData, name: e.target.value})} className={inputCls} placeholder="e.g. XL, 32, 34" />
                            </div>
                            <div>
                                <label className="text-[10px] font-black uppercase text-foreground/40 ml-1">Category (Optional)</label>
                            ...
                            {editItem && (
                                <div className="flex items-center gap-3 p-4 bg-background/50 rounded-2xl border border-border-theme mt-6">
                                    ...
                                    <label htmlFor="isActive-toggle" className="text-sm font-bold cursor-pointer select-none">
                                        Size is Active and Publicly Visible
                                    </label>
                                </div>
                            )}
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
                {filteredSizes.map(item => (
                    <div key={item._id} className="group bg-surface/40 hover:bg-surface border border-border-theme rounded-3xl p-6 transition-all flex flex-col justify-between h-40">
                            ...
                            <div className="flex justify-between items-start gap-4">
                                <div className="flex-1 min-w-0">
                                    ...
                                    <p className="text-[10px] text-foreground/40 font-bold truncate">
                                        Category: {item.category?.name || 'All Categories'}
                                    </p>
                                </div>
                        ...
                        <div className="relative z-10 flex items-center justify-between mt-auto">
                            <span className="text-[9px] font-black text-foreground/35 uppercase tracking-wider">Sort Order: {item.sortOrder || 0}</span>
                        </div>
                    </div>
                ))}
                {filteredSizes.length === 0 && (
                    <div className="col-span-full py-20 flex flex-col items-center justify-center gap-3 text-foreground/25 border border-dashed border-border-theme/40 rounded-3xl bg-surface/10">
                        <i className="ri-ruler-line text-4xl" />
                        <p className="text-sm font-black uppercase tracking-widest">No sizes found</p>
                    </div>
                )}
            </div>
        </div>
