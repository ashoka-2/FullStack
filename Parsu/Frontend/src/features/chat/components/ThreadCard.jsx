...
const ThreadCard = ({ thread, viewMode, onDelete }) => {
    ...
    return (
        <Link 
            ...
            onMouseEnter={() => triggerBlobChatSelect()}
            onClick={() => triggerBlobChatSelect()}
            className="group flex items-center justify-between p-4 bg-white dark:bg-[var(--bg-primary)] border border-zinc-200/90 dark:border-white/5 rounded-2xl hover:border-zinc-300 dark:hover:border-zinc-800 hover:bg-zinc-50 dark:hover:bg-zinc-800/10 shadow-2xs transition-all cursor-pointer"
        >
