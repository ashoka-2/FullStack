  ...
  return (
    <div className={`mt-10 sm:mt-14 relative mx-auto max-w-4xl w-full text-left ${className}`}>
      ...
      <div className="relative rounded-2xl sm:rounded-3xl border border-zinc-200/80 dark:border-white/10 bg-white/80 dark:bg-[#0c0e12]/85 backdrop-blur-2xl p-3.5 sm:p-6 shadow-2xl shadow-black/10 dark:shadow-black/60 transition-all duration-300">
        ...
        <div className="flex items-center justify-between px-2 pb-3 mb-3 border-b border-zinc-200/70 dark:border-white/10 gap-2">
          <div className="flex items-center gap-1.5 sm:gap-2 min-w-0">
            <span className="w-2.5 h-2.5 sm:w-3 sm:h-3 rounded-full bg-red-500/90 shadow-xs shrink-0" />
            <span className="w-2.5 h-2.5 sm:w-3 sm:h-3 rounded-full bg-amber-400/90 shadow-xs shrink-0" />
            <span className="w-2.5 h-2.5 sm:w-3 sm:h-3 rounded-full bg-emerald-400/90 shadow-xs shrink-0" />
            <span className="ml-1 sm:ml-2.5 font-mono text-[10px] sm:text-xs text-zinc-400 dark:text-zinc-500 select-none truncate max-w-[120px] xs:max-w-[180px] sm:max-w-none">
              https://parsuai.vercel.app/workspace
            </span>
          </div>

          <div className="flex items-center gap-1 sm:gap-1.5 px-2 sm:px-2.5 py-0.5 sm:py-1 rounded-full bg-[var(--accent-cyan)]/10 text-[var(--accent-cyan)] text-[9px] sm:text-[11px] font-bold uppercase tracking-wider shrink-0">
            <span className="w-1.5 h-1.5 rounded-full bg-[var(--accent-cyan)] animate-ping shrink-0" />
            <span>Live Terminal</span>
          </div>
        </div>

        {/* ── AI Model Selection Tabs ──────────────────────────────────────── */}
        <div className="flex items-center gap-1.5 sm:gap-2 overflow-x-auto pb-2.5 mb-3 border-b border-zinc-200/50 dark:border-white/5 scrollbar-none touch-pan-x">
          {models.map((m) => (
            <button
              key={m.id}
              onClick={() => setActiveModel(m.id)}
              className={`flex items-center gap-1.5 sm:gap-2 px-2.5 sm:px-3 py-1.5 rounded-xl text-xs font-semibold whitespace-nowrap transition-all cursor-pointer shrink-0 ${
                activeModel === m.id
                  ? 'bg-zinc-900 text-white dark:bg-white/10 dark:text-white shadow-xs border border-white/15'
                  : 'text-zinc-500 hover:text-zinc-900 dark:hover:text-white hover:bg-zinc-100 dark:hover:bg-white/5'
              }`}
            >
              <span className={`w-1.5 h-1.5 sm:w-2 sm:h-2 rounded-full ${activeModel === m.id ? 'bg-[var(--accent-cyan)]' : 'bg-zinc-400'}`} />
              <span>{m.name}</span>
              <span className="text-[9px] uppercase px-1.5 py-0.5 rounded bg-zinc-200 dark:bg-white/10 text-zinc-700 dark:text-zinc-300 hidden xs:inline">
                {m.badge}
              </span>
            </button>
          ))}
        ...
        <div className="p-3 sm:p-5 text-left bg-zinc-50/70 dark:bg-[#070809]/80 rounded-2xl border border-zinc-200/60 dark:border-white/5 font-mono text-xs">
          ...
          <div className="flex items-start gap-2.5 mb-3.5 text-zinc-800 dark:text-zinc-200 font-sans">
            ...
            <div className="bg-white dark:bg-zinc-900/90 border border-zinc-200 dark:border-white/10 rounded-xl px-3 py-2 sm:px-3.5 sm:py-2.5 w-full shadow-xs">
          ...
          <div className="mt-3.5 pt-3 flex flex-col xs:flex-row items-start xs:items-center justify-between gap-2 text-[11px] text-zinc-500 border-t border-zinc-200/40 dark:border-white/5">
            <span className="font-sans text-[10px] sm:text-xs">Ready to explore full multi-turn conversations and citations?</span>
            <Link
              to="/ai"
              className="w-full xs:w-auto px-3.5 py-1.5 rounded-lg bg-[var(--accent-cyan)] text-black font-bold text-xs shrink-0 hover:bg-[var(--accent-cyan-hover)] transition-all flex items-center justify-center gap-1 font-sans cursor-pointer shadow-sm hover:scale-[1.02] active:scale-[0.98]"
            >
