  ...
  return (
    <div className="space-y-8">
      ...
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
        <div>
          <h1 className="text-2xl font-bold tracking-tight text-white">API Usage & Quota Telemetry</h1>
      ...
      <section className="p-6 rounded-3xl bg-[#11131a]/80 border border-white/[0.08] shadow-sm backdrop-blur-xl space-y-6">
        <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3 border-b border-white/[0.05] pb-4">
          <div className="flex items-center gap-3">
            <div className="w-10 h-10 rounded-2xl bg-red-500/10 text-red-400 flex items-center justify-center">
              <RiMapPinLine size={20} />
            </div>
            <div>
              <h2 className="text-base font-bold text-white">Google Maps Platform Quota</h2>
              <p className="text-xs text-zinc-400">Maps JavaScript API • $200.00 Recurring Monthly Credit</p>
            </div>
          </div>
        ...
        <div className="space-y-2">
          <div className="flex items-center justify-between text-xs">
            <span className="text-zinc-400">
              Monthly Usage: <strong className="text-white font-mono">{maps?.usedThisMonth ?? 34}</strong> / {maps?.monthlyFreeLimit ?? 28500} loads
            </span>
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 pt-2">
          <div className="p-3.5 rounded-xl bg-white/[0.02] border border-white/[0.04]">
            ...
            <div className="text-lg font-bold text-white mt-1">28,500</div>
            <span className="text-[10px] text-zinc-400">loads every month</span>
          </div>
          ...
          <div className="p-3.5 rounded-xl bg-white/[0.02] border border-white/[0.04]">
            ...
            <span className="text-[10px] text-zinc-400">of $200.00 monthly</span>
          </div>
          ...
          <div className="p-3.5 rounded-xl bg-white/[0.02] border border-white/[0.04]">
            ...
            <span className="text-[10px] text-emerald-400">zero excess charges</span>
          </div>

          <div className="p-3.5 rounded-xl bg-white/[0.02] border border-white/[0.04]">
            <span className="text-[10px] uppercase font-bold text-zinc-500 tracking-wider">Recommended Cap</span>
            <div className="text-lg font-bold text-cyan-400 mt-1">900 / day</div>
            <span className="text-[10px] text-zinc-400">set in Google Console</span>
          </div>
        </div>
      </section>

      {/* Multi-Model AI API Consumption */}
      <section className="p-6 rounded-3xl bg-[#11131a]/80 border border-white/[0.08] shadow-sm backdrop-blur-xl space-y-5">
        <div className="flex items-center justify-between border-b border-white/[0.05] pb-4">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h2 className="text-base font-bold text-white">AI Language Model Utilization</h2>
              <p className="text-xs text-zinc-400">Estimated prompt tokens, invocation calls, and endpoint latency</p>
            </div>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          {ai.map((item, idx) => (
            <div
              key={idx}
              className="p-5 rounded-2xl bg-white/[0.02] border border-white/[0.05] hover:border-white/10 transition-colors space-y-3"
            >
              ...
              <div className="grid grid-cols-3 gap-2 pt-2 border-t border-white/[0.04] text-xs">
                <div>
                  <span className="text-[10px] text-zinc-500 block uppercase">Invocations</span>
                  <span className="font-mono font-bold text-white">{item.totalCalls.toLocaleString()}</span>
                </div>
                <div>
                  <span className="text-[10px] text-zinc-500 block uppercase">Tokens (Est.)</span>
                  <span className="font-mono font-bold text-cyan-400">{item.estimatedTokens.toLocaleString()}</span>
                </div>
                <div>
                  <span className="text-[10px] text-zinc-500 block uppercase">Latency</span>
                  <span className="font-mono font-bold text-zinc-300">{item.latency}</span>
                </div>
              </div>
            </div>
          ))}
