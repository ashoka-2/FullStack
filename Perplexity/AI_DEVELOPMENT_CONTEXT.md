...
- **Tavily Web Search Toggle**:
  ...
  - State persisted in `localStorage.getItem("perplexity_web_search")`.
...
- Respects custom voice, speed rate, and pitch from `localStorage` (`perplexity_tts_voice`, `perplexity_tts_rate`, `perplexity_tts_pitch`).
