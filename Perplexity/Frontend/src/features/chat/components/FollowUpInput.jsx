                ...
                <div className="w-full bg-zinc-50 dark:bg-[#121212] border border-zinc-200 dark:border-[#2d2e2e] focus-within:border-zinc-300 dark:focus-within:border-zinc-700 rounded-[28px] px-6 py-4 transition-all duration-300 shadow-[0_20px_50px_-20px_rgba(0,0,0,0.1)] dark:shadow-[0_40px_80px_-20px_rgba(0,0,0,0.9)]">
                    ...
                    {files.length > 0 && (
                        <div className="flex flex-wrap gap-2 mb-4 animate-in slide-in-from-bottom-2">
                            {files.map((file, i) => (
                                <div key={i} className="flex items-center gap-2 px-3 py-1.5 bg-zinc-100 dark:bg-[#1a1a1a] border border-zinc-200 dark:border-white/5 rounded-xl">
                                    {file.isLink ? <RiAttachment2 size={12} className="text-[#60A6AF]" /> : <RiFileTextLine size={12} className="text-[#60A6AF]" />}
                                    <span className="text-[11px] font-bold text-zinc-500 dark:text-zinc-400 truncate max-w-[120px]">{file.name}</span>
                                    <RiCloseLine size={14} className="text-zinc-400 dark:text-zinc-700 hover:text-red-400 cursor-pointer ml-1" onClick={() => removeFile(i)} />
                                </div>
                            ))}
                        </div>
                    )}
                    ...
                    <textarea 
                        ...
                        className="w-full bg-transparent border-none outline-none text-zinc-900 dark:text-zinc-100 text-[18px] md:text-[20px] placeholder:text-zinc-400 dark:placeholder:text-zinc-600 font-medium py-1 resize-none min-h-[40px] max-h-[160px] custom-scrollbar mb-3"
                    />
                    ...
                    <div className="flex items-center justify-between">
                        ...
                        <div className="flex items-center gap-2">
                            <div className="relative">
                                ...
                                {isUploadMenuOpen && (
                                    <div className="absolute bottom-full left-0 mb-4 w-56 bg-white dark:bg-[#1a1a1a] border border-zinc-200 dark:border-zinc-800 rounded-2xl p-2 shadow-2xl animate-in fade-in slide-in-from-bottom-2">
                                        <button onClick={() => fileInputRef.current.click()} className="w-full flex items-center gap-3 px-3 py-2.5 rounded-xl hover:bg-zinc-100 dark:hover:bg-zinc-800 text-zinc-700 dark:text-zinc-300 text-[13px] font-bold transition-all group">
                        ...
                        <div className="flex items-center gap-3">
                            ...
                            <button 
                                ...
                                className={`w-9 h-9 flex items-center justify-center rounded-full transition-all ${input.trim() || files.length > 0 ? 'bg-zinc-900 dark:bg-white text-white dark:text-black shadow-lg' : 'bg-zinc-100 dark:bg-zinc-800 text-zinc-300 dark:text-zinc-600 opacity-50'}`}
                            >
