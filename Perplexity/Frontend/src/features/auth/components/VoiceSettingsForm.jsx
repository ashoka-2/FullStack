...
const VoiceSettingsForm = ({ onSuccess }) => {
  ...
  const [selectedVoiceURI, setSelectedVoiceURI] = useState(() => {
    return localStorage.getItem('perplexity_tts_voice') || '';
  });
  const [rate, setRate] = useState(() => {
    return parseFloat(localStorage.getItem('perplexity_tts_rate') || '1');
  });
  const [pitch, setPitch] = useState(() => {
    return parseFloat(localStorage.getItem('perplexity_tts_pitch') || '1');
  });
      ...
      if (availableVoices.length > 0) {
        ...
        if (!selectedVoiceURI) {
          ...
          if (defaultVoice) {
            ...
            localStorage.setItem('perplexity_tts_voice', defaultVoice.voiceURI);
          }
        }
      }
    };
  ...
  const handleVoiceChange = (e) => {
    ...
    localStorage.setItem('perplexity_tts_voice', newURI);
  ...
  const handleRateChange = (e) => {
    ...
    setRate(newRate);
    localStorage.setItem('perplexity_tts_rate', String(newRate));
  };
  ...
  const handlePitchChange = (e) => {
    ...
    setPitch(newPitch);
    localStorage.setItem('perplexity_tts_pitch', String(newPitch));
  };
